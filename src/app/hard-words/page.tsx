...
import { Loader2, ArrowRight, ArrowLeft, Copy, Trash2 } from "lucide-react";
...
// This component contains the main client-side logic and rendering
const HardWordsContent = () => {
    const [hardWords, setHardWords] = useState<HardWord[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const { toast } = useToast();
    const { user, loading: authLoading } = useAuth();
    const router = useRouter(); // useRouter is now inside this component
...
'); // Corrected newline escape
    ...
    // Conditional rendering for loading and not authenticated states
    ...
    // Main content when words are loaded or empty
    return (
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-4 rtl:sm:space-x-reverse w-full max-w-lg">
                 <Button variant="secondary" onClick={() => router.push('/')} className="w-full sm:w-auto">
                   <ArrowRight className="w-4 h-4 mr-2 rtl:ml-2"/>
                   العودة للإدخال
                 </Button>
...
// The main page component remains the default export and wraps the content in Suspense
const HardWordsPage = () => {
  return (
